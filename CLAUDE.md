...
**不要把兩邊的 backend / domain 邏輯混用** — 回放器的資料來源是 `dw_analysis_record.search_event_*` 表(dataform 產出,
...
- **`test_exp` 一律當字串處理(10 碼制)** — KKDay search API v3 用 `test_exp` 選演算法版本;10 碼制後可能含前導零(e.g. `0000000001`),所以 **前端 state → Pydantic → fetch body → DB 全鏈都是字串,不做合法性驗證**(前端帶什麼送什麼)。相容層:main.py 的 `TestExpStr`(`BeforeValidator(lambda v: v if v is None else str(v))`,None-guard 必要 — 裸 `str` 會把 `Optional` 欄位的 `None` 變成 `"None"`)讓舊 client 送 int 也能收(Pydantic v2 預設不 coerce int→str);`kkday_api` body 端 `str(test_exp)` 保底。`ab_check_runs.version_a/b` 為 TEXT 欄位 — 舊 DB 是 INTEGER affinity(會把前導零字串吃成 int),`init_schema()` 偵測到就自動 rebuild 表 migrate(`BEGIN…COMMIT` 原子化;rebuild 前先寫一次性檔案級備份 `history.db.pre-text-migration.bak`,gitignored,救「成功執行了錯誤轉換」的情境);resume 的 A/B 比對兩邊 `str()` normalize 避免舊 int run 續跑誤判。**Code rollback 容舊**:舊 code 跑在新 TEXT schema 上安全(SQLite affinity 自動轉型、不認識的 device_id 欄留 NULL),唯一退化是舊 code 的 resume 比對會 int vs str 誤判 → fallback 全跑。前端輸入框(SettingsPanel / HomePage 欄頭 / ABCheckRunPanel)都是 free-text,無 parseInt、無 maxLength 過濾。
- **`device_id` 由前端帶入(個性化搜尋巡檢)** — SettingsPanel 有 Device ID 欄位(空 = 用後端 `kkday_api.DEFAULT_DEVICE_ID`,即 env `KKDAY_SEARCH_DEVICE_ID` fallback 舊寫死值),由 `AppContext.deviceId` 集中管理,流經 `/api/unified-search`、`/api/ab-check/start`、`/api/compare`、`/api/batch/run`。`ab_check._fetch_results` cache key 含 device_id(不同 device 個性化結果不同,不可共用);`ab_check_runs` 存 run-level `device_id`(NULL = 後端預設),**resume 跟 locale 一樣自動沿用 parent 的 device_id**。UI 顯示比照 locale 三處(RunStatusBar / ABCheckRunPanel 進度行 / ABCheckHistoryTable,截斷顯示 hover 看全值)。
...
- **AB-check run 的 locale 在第一次啟動就釘住** — `ab_check_runs` 表存 lang/locale/channel(`CREATE TABLE` 與 `ALTER TABLE ADD COLUMN` 雙寫,既存 DB 啟動時自動 migrate)。**Resume 自動沿用 parent locale**:`ab_check_runner.start_run` 收到 `resume_run_id` 時,從 DB 讀 parent 的 lang/locale/channel 蓋掉 caller 傳入值(log info 紀錄),確保續跑的 ok rows 與新跑 rows 不會跨 locale 混合。`_copy_parent_done_rows` 因此不用加 bail-on-mismatch guard。UI 三處顯示 run-level locale:`RunStatusBar`(cancelled/interrupted/done/failed 區塊,interrupted 附「(續跑沿用)」)、`ABCheckRunPanel` 進度行(running 中)、`ABCheckHistoryTable`(歷史表 locale 欄)。
